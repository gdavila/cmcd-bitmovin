...
function setupPlayerWithCmcd() {
  ...
  const CmcdQueryValidator = window["cmcd-validator-debug"].CMCDQueryValidator;
...
function logValidator(data){
  ...
  while (document.getElementsByClassName('logValidator').logValidator.childElementCount > 10){
    document.getElementsByClassName('logValidator').logValidator.removeChild(document.getElementsByClassName('logValidator').logValidator.children[9])
  }
  $('<p class="log-message cmcd-log"></p>')
  .append(Date.now() +': Valid '+ JSON.stringify (data.valid)+ ' - '+JSON.stringify(data.parsedData))
  .prependTo('#logValidator');
